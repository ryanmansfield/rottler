...
CSV.foreach(work_orders_filepath, **csv_options) do |row|
  ...
  # Loop Through Tech TS and find Time Slot the Work Order Belongs to
  technician.time_slots.each do |time_slot|
    ...
    if range.include?(workorder.start_time) && (time_slot.start_time == workorder.start_time || time_slot.end_time == workorder.end_time)
      ...
      TimeSlot.create(
        start_time: workorder.start_time, end_time: workorder.end_time, duration: workorder.duration,
        technician_id: technician.id, work_order_id: workorder.id, is_booked: true
      )
    elsif range.include?(workorder.start_time)
      # create time slot before
      TimeSlot.create(
        start_time: time_slot.start_time, end_time: workorder.start_time,
        duration: (workorder.start_time.to_i - time_slot.start_time.to_i) / 60, technician_id: technician.id
      )
      # create wo time slot
      TimeSlot.create(
        start_time: workorder.start_time, end_time: workorder.end_time,
        duration: workorder.duration, technician_id: technician.id, work_order_id: workorder.id, is_booked: true
      )
      # create time slot after
      TimeSlot.create(
        start_time: workorder.end_time, end_time: time_slot.end_time,
        duration: (time_slot.end_time.to_i - workorder.end_time.to_i) / 60, technician_id: technician.id
      )
